...
Open the **Settings** and, in the **Network** tab configure Interface 1 as **Internal**. If we now boot up the server we can see it picks ip an IP address. If we check the route you will see it sends its requests through the gateway.
```
ifconfig
ip route show
```
...
Now we need to configure the second network interface `eth1`.




Now we will make the first virtual machine (VM). This will be the one we clone to make clients later so we will only use the basic settings.
